...
Ep.Session.reopen({
  ...
  merge: function(model, strategy) {
    ...
    if(!get(model, 'hasErrors')) {
      ...
      // after a successful merge we update
      // the shadow and remove the backup
      shadows.add(model.copy());
      flushing.remove(model);
    } else {
      this.suspendDirtyChecking(function() {
        merged = this.mergeErrors(model, strategy);
      }, this);
      // in the event of an error we reset the
      // shadow to be the flushing copy
      shadows.add(flushing.getModel(model));
    }
    return merged;
  },
