...
Ep.Session = Ember.Object.extend({
  ...
  */
  ...
  /**
    Rollbacks a model or a list of models in this session. It can be called without arguments to
    rollback all dirty models in the session:

    ```
      session.rollback(); // is equivalent to session.rollback(session.get('shadows'))
    ```

    Or you can pass in a model or multiple models:

    ```
      session.rollback(model);
      session.rollback([model1, model2]); //is equivalent to : session.rollback(model1, model2)
    ```

    @method rollback
    @param {Ep.Model} models A model or list of models
  */
  //TODO deep rollback on relationships
  rollback: function (models) {
    var session = this,
        processModel = function (model) {
          var shadow;
          Ember.assert('Cannot rollback an undefined model', model !== undefined);
          shadow = session.getShadow(model);
          shadow.copyAttributes(model);
          //remove from dirty/originals models
          session.shadows.remove(model);
          session.originals.remove(model);
          //force compute dirty property in the model
          model.propertyDidChange('isDirty');
        };

    if (arguments.length === 0) {
      //if no args, operate on all dirty models
      models = this.shadows.map(function(model) {
        return session.models.getModel(model);
      });
    } else {
