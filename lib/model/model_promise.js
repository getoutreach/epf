...
Ep.ModelPromise = Ember.ObjectProxy.extend(Ep.ModelMixin, Ember.DeferredMixin, {
  ...
  merge: function(graph, strategy) {
    var content = get(this, 'content');
    if(content) {
      return graph.merge(content, strategy);
    }
    var dest;
    if(get(this, 'hasIdentifiers')) {
      dest = graph.getModel(this);
    }
    if(!dest) {
      ...
      this.then(function(model) {
        dest.resolve(graph.merge(model, strategy));
      }, function(err) {
        dest.reject(err);
        throw err;
      });
...
Ep.LazyModel = Ep.ModelPromise.extend({
  ...
  // TODO: re-use object if not associated with session a la normal models?
  merge: function(graph, strategy) {
    var content = get(this, 'content');
    if(content) {
      return graph.merge(content, strategy);
