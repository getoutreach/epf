...
Orm.SessionMixin = Ember.Mixin.create({
...
Orm.Session = Ember.Object.extend(Orm.SessionMixin, {
  ...
  create: function(type) {
    ...
    // TODO: move setting session to add
    set(model, 'session', this);
    this.add(model);
    return model;
  },
  ...
  deleteModel: function(model) {
    // TODO: handle with record array manager
  ...
  find: function(type, query) {
    ...
    return this.adapter.find(type, query).then(function(models) {
      var merged = Orm.ModelArray.create({content: []});
