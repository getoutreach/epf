...
Orm.Adapter = Ember.Object.extend(DS._Mappable, {
  ...
  isDirty: function(model) {
    ...
    // TODO think about the case where the id is explicitly set, but not in store
    // e.g. a backend where the ids are not auto-generated
    var dirty = false;

    model.eachAttribute(function(name, meta) {
      if(dirty) return;
      dirty = get(model, name) !== get(cached, name);
    });

    return dirty || this.isDirtyFromRelationships(model, cached);
  },

  isDirtyFromRelationships: mustImplement("isDirtyFromRelationships")
