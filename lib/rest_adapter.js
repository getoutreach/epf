  ...
  */
  ...
  _createDependencyGraph: function(session) {
    ...
    session.get('models').forEach(function(model) {
      ...
      } else { // TODO: should we not do this if deleted?
      ...
      // in thise case we rely on the parent being marked dirty based on
      // a change in the embedded has many
      if(this.isEmbedded(model) && !get(model, 'isNew')) {
    ...
    // TODO: these two passes are really ugly, we are doing
    // it because isRoot() below is recursive
  ...
  isDirtyFromRelationships: function(model, cached, relDiff) {
    ...
    for(var i = 0; i < relDiff.length; i++) {
      var diff = relDiff[i];
      if(diff.type === 'belongsTo') {
        return true;
      } else if(diff.type === 'hasMany') {
        var embeddedType = serializer.embeddedType(model.constructor, diff.name);
        // TODO: allow adapter configuration for non-relational databases
        if(embeddedType) return true;
      }
    }
    return false;
  },
