...
Ep.RelationshipChange = function(options) {
...
Ep.RelationshipChangeAdd = function(options){
  Ep.RelationshipChange.call(this, options);
};

Ep.RelationshipChangeRemove = function(options){
  Ep.RelationshipChange.call(this, options);
};

/** @private */
Ep.RelationshipChange.create = function(options) {
  return new Ep.RelationshipChange(options);
};

/** @private */
Ep.RelationshipChangeAdd.create = function(options) {
  return new Ep.RelationshipChangeAdd(options);
};

/** @private */
Ep.RelationshipChangeRemove.create = function(options) {
  return new Ep.RelationshipChangeRemove(options);
};
...
Ep.RelationshipChange._createChange = function(options){
  if(options.changeType === "add"){
    return Ep.RelationshipChangeAdd.create(options);
  }
  if(options.changeType === "remove"){
    return Ep.RelationshipChangeRemove.create(options);
  }
};


Ep.RelationshipChange.determineRelationshipType = function(recordType, knownSide){
...
Ep.RelationshipChange.createChange = function(firstRecordReference, secondRecordReference, store, options){
  // Get the type of the child based on the child's client ID
  var firstRecordType = firstRecordReference.type, changeType;
  changeType = Ep.RelationshipChange.determineRelationshipType(firstRecordType, options);
  if (changeType === "oneToMany"){
    return Ep.OneToManyChange.createChange(firstRecordReference, secondRecordReference, store, options);
  }
  else if (changeType === "manyToOne"){
    return Ep.OneToManyChange.createChange(secondRecordReference, firstRecordReference, store, options);
  }
  else if (changeType === "oneToNone"){
    return Ep.OneToNoneChange.createChange(firstRecordReference, secondRecordReference, store, options);
  }
  else if (changeType === "manyToNone"){
    return Ep.ManyToNoneChange.createChange(firstRecordReference, secondRecordReference, store, options);
  }
  else if (changeType === "oneToOne"){
    return Ep.OneToOneChange.createChange(firstRecordReference, secondRecordReference, store, options);
  }
  else if (changeType === "manyToMany"){
    return Ep.ManyToManyChange.createChange(firstRecordReference, secondRecordReference, store, options);
  }
};

/** @private */
Ep.OneToNoneChange.createChange = function(childReference, parentReference, store, options) {
  var key = options.key;
  var change = Ep.RelationshipChange._createChange({
...
/** @private */
Ep.ManyToNoneChange.createChange = function(childReference, parentReference, store, options) {
  var key = options.key;
  var change = Ep.RelationshipChange._createChange({
...
/** @private */
Ep.ManyToManyChange.createChange = function(childReference, parentReference, store, options) {
...
/** @private */
Ep.OneToOneChange.createChange = function(childReference, parentReference, store, options) {
  var key;
...
Ep.OneToOneChange.maintainInvariant = function(options, store, childReference, key){
  if (options.changeType === "add" && store.recordIsMaterialized(childReference)) {
    ...
    if (oldParent){
      var correspondingChange = Ep.OneToOneChange.createChange(childReference, oldParent.get('_reference'), store, {
...
/** @private */
Ep.OneToManyChange.createChange = function(childReference, parentReference, store, options) {
  var key;
  ...
  if (options.parentType) {
    key = options.parentType.inverseFor(options.key).name;
    Ep.OneToManyChange.maintainInvariant( options, store, childReference, key );
  } else if (options.key) {
    key = options.key;
  } else {
    Ember.assert("You must pass either a parentType or belongsToName option to OneToManyChange.forChildAndParent", false);
...
Ep.OneToManyChange.maintainInvariant = function(options, store, childReference, key){
  var child = childReference.record;

  if (options.changeType === "add" && child) {
    var oldParent = get(child, key);
    if (oldParent){
      var correspondingChange = Ep.OneToManyChange.createChange(childReference, oldParent.get('_reference'), store, {
          parentType: options.parentType,
          hasManyName: options.hasManyName,
          changeType: "remove",
          key: options.key
...
Ep.OneToManyChange.ensureSameTransaction = function(changes){
  var records = Ember.A();
  ...
  return Ep.Transaction.ensureSameTransaction(records);
};

Ep.RelationshipChange.prototype = {
  ...
  */
  ensureSameTransaction: function() {
    ...
    var transaction = Ep.Transaction.ensureSameTransaction([child, parentRecord]);
...
Ep.RelationshipChangeAdd.prototype = Ember.create(Ep.RelationshipChange.create({}));
Ep.RelationshipChangeRemove.prototype = Ember.create(Ep.RelationshipChange.create({}));

Ep.RelationshipChangeAdd.prototype.changeType = "add";
Ep.RelationshipChangeAdd.prototype.sync = function() {
...
Ep.RelationshipChangeRemove.prototype.changeType = "remove";
Ep.RelationshipChangeRemove.prototype.sync = function() {
  var secondRecordName = this.getSecondRecordName(),
      firstRecordName = this.getFirstRecordName(),
      firstRecord = this.getFirstRecord(),
      secondRecord = this.getSecondRecord();
