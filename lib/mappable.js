...
/**
  ...
  For example, the Ep.Adapter class can behave like a map, with
  more semantic API, via the `map` API:

    Ep.Adapter.map('App.Person', { firstName: { key: 'FIRST' } });
  ...
  * `transformMapValue` - allows a value (for example, a class that
  ...
  method built using the `generateMapFunctionFor` method:
    ...
    Ep.Adapter.reopenClass({
      map: Ep.Mappable.generateMapFunctionFor('attributes', function(key, newValue, map) {
...
Ep._Mappable = Ember.Mixin.create({
  createInstanceMapFor: function(mapName) {
    var instanceMeta = Ember.metaPath(this, ['Ep.Mappable'], true);
  ...
  _copyMap: function(mapName, klass, instanceMap) {
    var classMeta = Ember.metaPath(klass, ['Ep.Mappable'], true);
...
Ep._Mappable.generateMapFunctionFor = function(mapName, transform) {
  return function(key, value) {
    var meta = Ember.metaPath(this, ['Ep.Mappable'], true);
