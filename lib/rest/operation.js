...
Ep.Operation = Ember.Object.extend({
  ...
  perform: function(forceUpdate) {
    ...
    return promise.then(function(serverModel) {
      // Resolve a copy if the adapter returns the
      // same model (is the case when the server returns nothing)
      if(serverModel === model) {
        serverModel = model.copy();
      }
      // ensure the clientRev is set on the returned model
      // 0 is the default value
      if(get(serverModel, 'clientRev') === 0) {
