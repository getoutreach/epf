...
Ep.RestAdapter = Ep.Adapter.extend({
  ...
  remoteCall: function(context, name, params) {
    ...
    return this.ajax(url, method,{
      ...
      Ember.run(adapter, 'didError', xhr, context);
      throw xhr;
    });
  },
  ...
  flush: function(session) {
    ...
    // we need to be sure that all embedded parents are copied into the
    // dirty set
    var embeddedRelations = this.findEmbeddedRelations(models, session);
    embeddedRelations.forEach(function(rel) {
      if(!models.contains(rel)) {
        var copy = rel.copy()
        models.add(copy);
        shadows.add(copy);
      }
  ...
  */
  ...
  findEmbeddedRelations: function(models, session) {
    var res = Ep.ModelSet.create();
    models.forEach(function(model) {
      var parent;
