...
Ep.RestAdapter = Ep.Adapter.extend(EmbeddedHelpersMixin, {
  ...
  */
  ...
  processData: function(data, callback, binding) {
    var serializer = this.serializerFor('payload'),
    ...
    payload.forEach(function(model) {
      ...
      if (callback) callback.call(binding || this, model);
    }, this);
  ...
  didError: function(xhr, model) {
    ...
    if (xhr.status === 422) { // validation error
      var json = JSON.parse(xhr.responseText),
          serializer = this.serializerForModel(model),
          validationErrors = this.extractValidationErrors(get(model, 'type'), json);
