...
Ep.RestAdapter = Ep.Adapter.extend({
  ...
  remoteApply: function(context, name, args) {
    ...
    if(typeof context === 'function') {
      ...
      url = this.buildURL(this.rootForType(context.constructor), get(context, 'id'));
    }

    data = {};
    data[root] = get(this, 'serializer').serialize(model);

    return this.ajax(this.buildURL(root, id), "POST",{
      data: data
    }).then(function(json){
