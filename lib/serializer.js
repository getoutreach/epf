...
Ep.Serializer = Ember.Object.extend({
  ...
  createModel: function(type) {
    // TODO: use container to instantiate?
    return type.create();
  },
  ...
  // TODO: return map instead of having side effects
  ...
  deserializeHasMany: function(name, record, data, relationship) {
    var type = record.constructor,
        key = this._keyForHasMany(type, relationship.key),
        embeddedType = this.embeddedType(relationship, data),
        value = this.extractHasMany(type, data, key);

    if(embeddedType) {
      this.deserializeEmbeddedHasMany(name, record, value, relationship);
    } else { // non-embedded
      this.deserializeLazyHasMany(name, record, value, relationship);
    }
  },
  ...
  deserializeBelongsTo: function(name, record, hash, relationship) {
    var type = record.constructor,
        key = this._keyForBelongsTo(type, relationship.key),
        embeddedType = this.embeddedType(relationship, hash),
        value;
    ...
    if(embeddedType) {
      ...
      this.deserializeEmbeddedBelongsTo(name, record, value, relationship);
    } else {
      value = this.extractBelongsTo(type, hash, key);
      this.deserializeLazyBelongsTo(name, record, value, relationship);
    }
  },
  ...
  */
  ...
  /**
    ...
    * Add the record's relationships to the serialized form. By default, `serialize` calls
    ...
    @method serialize
    @param {Ep.Model} record the record to serialize
  ...
  */
  ...
  /**
    ...
    * the serialized id (after calling the `serializeId` hook)
    ...
    @method addType
    @param {any} data the serialized representation that is being built
    @param {Ep.Model subclass} type the type of the record
  */
  addType: Ember.K,
  ...
  */
  ...
  /**
    ...
    is no default implementation. You can see `Ep.JSONSerializer`
    ...
    * **type** a subclass of Ep.Model that is the type of the
      relationship. This is the first parameter to Ep.belongsTo
    * **options** the options passed to the call to Ep.belongsTo
  ...
  */
  ...
  /**
    ...
    * **type** a subclass of Ep.Model that is the type of the
      relationship. This is the first parameter to Ep.hasMany
    * **options** the options passed to the call to Ep.hasMany
    * **kind** always `hasMany`
  ...
  */
  ...
  /**
    ...
    ```javascript
    App.MySerializer = Ep.Serializer.extend({
      // ...
    ...
    @param {Ep.Model subclass} type the type of the record with
  ...
  */
  ...
  typeFromAlias: function(alias) {
    ...
    // TODO: revisit type configuration
